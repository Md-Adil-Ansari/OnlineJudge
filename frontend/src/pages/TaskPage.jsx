    ...
    <div className="flex flex-col min-h-screen bg-gray-950 text-white">
        ...
        <div className="min-h-screen bg-gray-800 flex flex-col items-center py-10">
            ...
            <div className="container mx-auto flex flex-col lg:flex-row space-y-8 lg:space-y-0 lg:space-x-8 px-4">
                <div className="flex-1 bg-gray-700 shadow-lg rounded-lg p-6">
                    <h1 className="font-bold text-3xl mb-4">{task.name}</h1>
                    <h2 className="font-bold text-2xl mb-2">Problem Statement</h2>
                    <div className="mb-4 p-4 bg-gray-950 rounded" style={{ whiteSpace: 'pre-wrap' }}>{task.statement}</div>
                    <h2 className="font-bold text-2xl mb-2">Format</h2>
                    <div className="mb-4 p-4 bg-gray-950 rounded" style={{ whiteSpace: 'pre-wrap' }}>{task.format}</div>
                    <h2 className="font-bold text-2xl mb-2">Constraints</h2>
                    <div className="mb-4 p-4 bg-gray-950 rounded" style={{ whiteSpace: 'pre-wrap' }}>{task.constraints}</div>
                    <h2 className="font-bold text-2xl mb-2">Test Cases</h2>
                    <div>
                        {task.testcases.map((testcase, index) => (
                            <div key={index} className="mb-4 p-4 bg-gray-950 rounded">
                    ...
                    <div className="bg-gray-700 shadow-lg rounded-lg p-6">
                        ...
                        <div className="mb-4 flex space-x-2">
                            <button
                                ...
                                className={`px-4 py-2 rounded ${language === 'c' ? 'bg-yellow-600 text-white' : 'bg-white text-gray-700'}`}
                            >
                                C
                            </button>
                            <button
                                onClick={() => setLanguage('cpp')}
                                className={`px-4 py-2 rounded ${language === 'cpp' ? 'bg-yellow-600 text-white' : 'bg-white text-gray-700'}`}
                            >
                                C++
                            </button>
                            <button
                                onClick={() => setLanguage('java')}
                                className={`px-4 py-2 rounded ${language === 'java' ? 'bg-yellow-600 text-white' : 'bg-white text-gray-700'}`}
                            >
                                Java
                            </button>
                            <button
                                onClick={() => setLanguage('python')}
                                className={`px-4 py-2 rounded ${language === 'python' ? 'bg-yellow-600 text-white' : 'bg-white text-gray-700'}`}
                            >
                                Python
                            </button>
                        </div>
                        <div className="mb-4 text-white ">
                            <textarea
                                ...
                                className="w-full bg-gray-800 h-64 border border-gray-300 rounded-md p-2 font-mono text-sm"
                                placeholder="Write your code here..."
                                style={{ minHeight: '50vh' }}
                            />
                        </div>
                        <div className="mb-4 text-white bg-700">
                            <label className="block mb-2">Input:</label>
                            <textarea
                                value={input}
                                onChange={(e) => setInput(e.target.value)}
                                className="w-full bg-gray-800 h-32 border border-gray-300 rounded-md p-2 font-mono text-sm"
                        ...
                        <div className="mb-4 text-white bg-700">
                            ...
                            <textarea
                                value={output}
                                readOnly
                                className="w-full h-32 border bg-gray-800 border-gray-300 rounded-md p-2 font-mono text-sm "
                        ...
                        <div className="flex space-x-4">
                            <button
                                ...
                                className="w-full py-2 px-4 bg-yellow-600 hover:bg-yellow-700 text-white font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-opacity-75 transition duration-150 ease-in-out"
                            >
                                Compile
                            </button>
                            <button
