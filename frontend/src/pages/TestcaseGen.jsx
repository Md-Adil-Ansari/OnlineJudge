...
function TestcaseGen() {
  ...
  return (
    <>
      ...
      <Navbar />
      <div className="flex flex-wrap mt-7 mb-6 bg-white rounded shadow p-4 sm:w-3/4 lg:w-1/2 mx-auto">
        ...
        {testcases.map((testcase, testcaseIndex) => (
          <div key={testcaseIndex} className="w-full px-3 mb-6 bg-gray-200 rounded shadow p-4">
            ...
            <button type="button" onClick={() => handleDeleteTestcase(testcaseIndex)}
              className="px-4 py-2 font-bold text-white bg-yellow-500 rounded hover:bg-yellow-700 focus:outline-none focus:shadow-outline">Delete Test Case</button>
          </div>
        ))}
        <button type="button" onClick={handleAddTestcase}
          className="w-full px-4 py-2 mt-3 font-bold text-white bg-yellow-500 rounded hover:bg-yellow-700 focus:outline-none focus:shadow-outline">Add Test Case</button>
        <button type="button" onClick={handleSubmit}
          className="w-full px-4 py-2 mt-3 font-bold text-white bg-gray-500 rounded hover:bg-gray-700 focus:outline-none focus:shadow-outline">Submit</button>
      </div>
      <Footer />
      </div>
    </>
